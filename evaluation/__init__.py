...
if __name__ == '__main__':
    ...
    parser.add_argument('-se', '--samples-epochs', type=int, default=None, help='Number of epoch from which the samples were generated.')
    ...
    if args.samples_epochs is not None:
        pred_args += ['--samples-epoch', args.samples_epochs]

    os.system(to_string(pred_args))

    vis_args = ['python evaluation/visualization.py', '--name', args.name, '--dset', args.dset]
    if args.samples_epochs is not None:
        vis_args += ['--samples-epoch', args.samples_epochs]
    if args.vis_type:
